...
- [x] Предрезолюция маршрутов DC (проба прямого пути при старте и раз в 30 мин, преселект фолбэка при блокировке).
...
* 🔌 [main.go](src/main.go) — Точка входа, парсинг флагов, запуск TCP-слушателя и обработка системных сигналов.
* 🌁 [bridge.go](src/bridge.go) — Реализация пересылки данных (pipe/bridge) между клиентом и Telegram.
* ☁️ [cfproxy.go](src/cfproxy.go) — Функции для работы с Cloudflare, включая тестирование доступности.
* 🔐 [crypto.go](src/crypto.go) — Логика шифрования AES-CTR-128 и расшифровка обфусцированного MTProto.
* 📡 [dns.go](src/dns.go) — Кастомный резолвер имен для обхода отсутствия системного резолвера.
* ✂️ [splitter.go](src/splitter.go) — Сплиттер пакетов для правильной нарезки MTProto данных во фреймы WebSocket.
* 🌐 [websocket.go](src/websocket.go) — Инициализация WSS соединения с поддержкой кастомных заголовков.
