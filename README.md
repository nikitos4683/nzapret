...
По умолчанию режим стека определяется автоматически при первом запуске: если на устройстве виден рабочий IPv6, сохраняется `auto`, иначе модуль сам переключается в `ipv4-only`. После ручной смены через WebUI или CLI выбор пользователя больше не перезаписывается автоматически.
