...
const translations = {
    ru: {
        ...
        user_list: {
            ...
            subtitle: 'Добавляйте или удаляйте домены, затем нажимайте Сохранить. Кнопка Обновить заново читает сохранённый список с устройства, а nfqws2 подхватывает изменения hostlist без перезапуска.',
            ...
            comments_not_added: 'Комментарии здесь не добавляются',
        ...
        network: {
            ...
            auto_label: 'Авто',
            auto_meta: 'Использовать IPv4 и держать IPv6 наготове, когда он поддерживается.',
            ipv4_only_label: 'Только IPv4',
            ipv4_only_meta: 'Не создавать правила ip6tables и IPv6 bind-fix при рестарте.',
            dirty_none: 'Несохранённых изменений нет.',
            ...
            mode_auto_fallback: 'Авто (fallback только IPv4)'
        },
        private_dns: {
            title: 'Private DNS',
            subtitle_html: 'Управляйте системным Android Private DNS. При первом запуске службы nzapret инициализирует <code>{{default_hostname}}</code>, если на устройстве уже не настроен другой provider hostname.',
            off_label: 'Выкл',
            off_meta: 'Отключить Private DNS для системного Android resolver.',
            auto_label: 'Авто',
            auto_meta: 'Пусть Android использует шифрованный DNS, когда текущая сеть его поддерживает.',
            default_meta: 'Сразу применить provider hostname по умолчанию для nzapret.',
            hostname_placeholder: 'dns.example.com',
            apply_hostname: 'Применить hostname',
            status_initial: 'Статус Private DNS появится здесь после первого обновления.',
            status_unavailable: 'Управление Private DNS недоступно, потому что в этой сборке Android отсутствует команда settings.',
            status_init_pending: 'При первом запуске службы nzapret выставит {{default_hostname}}, если уже не настроен другой provider hostname.',
            status_off: 'Private DNS отключён для системного Android resolver.',
            status_default_active: '{{hostname}} активен как provider по умолчанию для nzapret. Изменения применяются сразу.',
            status_custom_active: 'Сейчас активен пользовательский provider {{hostname}}. Изменения применяются сразу.',
            status_custom_unspecified: 'Сейчас активен пользовательский provider hostname. Изменения применяются сразу.',
            status_auto: 'Автоматический режим позволяет Android использовать шифрованный DNS, когда текущая сеть его поддерживает. Изменения применяются сразу.',
            updated: 'Private DNS обновлён',
            invalid_hostname: 'Введите корректный provider hostname',
            loading_disable: 'Отключение Private DNS...',
            loading_auto: 'Переключение Private DNS в автоматический режим...',
            loading_default: 'Применение {{hostname}}...',
            loading_hostname: 'Применение hostname Private DNS...',
            mode_off: 'Выкл',
            mode_auto: 'Авто',
        ...
        diagnostics: {
            ...
            empty_text: 'Запустите проверку, чтобы подтвердить процесс, цепочки firewall и другие live-сигналы перед разбором сырых логов.',
            ...
            details: {
                ...
                ip6tables_missing_fallback: 'команда отсутствует, авто-режим перейдёт в fallback только IPv4',
                ...
                network_auto_fallback: 'Авто (fallback только IPv4)',
                ...
                private_dns_hostname_custom: 'пользовательский provider hostname ({{hostname}})',
        ...
        logs: {
            ...
            runtime_tab: 'nfqws2',
            events_tab: 'События',
            loading_runtime: 'Загрузка runtime-лога...',
            runtime_empty: 'Runtime-лог пуст.',
